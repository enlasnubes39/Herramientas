...
(async function() {
    ...
    if (!viewer) {
        alert("Error: Asegúrate de tener el PDF abierto en pantalla.");
        return;
    }
    ...
    win.document.write(`
        <html>
        <head>
            <title>Captura: PDF Liberado</title>
            <style>
                ...
                @media print {
                    ...
                    .page-img { width: 100vw; height: 100vh; object-fit: contain; page-break-after: always; }
                    #status { display: none !important; }
                }
            </style>
        </head>
        <body>
            <div id="status" style="position:fixed; top:10px; left:10px; background:black; color:lime; padding:10px; z-index:9999; font-family:monospace;">
                Cargando páginas...
            </div>
    ...
    for (let i = 1; i <= totalPages; i++) {
        status.innerText = `[PROCESANDO] Página ${i} de ${totalPages}...`;
        viewer.page = i;
        await new Promise(r => setTimeout(r, 2200));

        const canvas = viewer.pdfViewer.getPageView(i - 1).canvas;
        if (canvas) {
